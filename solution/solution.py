...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        total_tokens += count_tokens(user_message, OPENAI_MODEL)
        total_tokens += count_tokens(assistant_reply, OPENAI_MODEL)
        costs = estimate_cost(user_message, assistant_reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        rows.append(
            ...
            f"{shorten(result['gpt4o_response'])} | "
            f"{shorten(result['mini_response'])} | "
